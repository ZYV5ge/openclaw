...
struct MacNodeModeCoordinatorTests {
    @Test @MainActor func `irrelevant defaults churn does not invalidate the node endpoint`() async throws {
        ...
        defer {
            ...
            if let savedComputerControl { defaults.set(savedComputerControl, forKey: computerControlEnabledKey) }
        ...
        let before = coordinator.generationsForTesting()
        notificationCenter.post(
            name: UserDefaults.didChangeNotification,
            object: defaults)
        try await Task.sleep(for: .milliseconds(20))
        let afterIrrelevantChange = coordinator.generationsForTesting()
        #expect(afterIrrelevantChange.endpointAttempt == before.endpointAttempt)

        defaults.set(true, forKey: cameraEnabledKey)
        notificationCenter.post(
            name: UserDefaults.didChangeNotification,
            object: defaults)
        try await Task.sleep(for: .milliseconds(20))
