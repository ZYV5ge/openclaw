...
describe("gateway server chat", () => {
  ...
  test("chat.history uses the prepared catalog without starting provider discovery", async () => {
    openDirectChatSession();
    try {
      ...
      const context = createDirectChatContext({
        readPreparedGatewayModelCatalog: vi
          .fn<NonNullable<GatewayRequestContext["readPreparedGatewayModelCatalog"]>>()
          .mockResolvedValue(catalog),
        loadGatewayModelCatalogSnapshot: vi
          .fn<GatewayRequestContext["loadGatewayModelCatalogSnapshot"]>()
          .mockRejectedValue(new Error("full catalog discovery must not run for chat.history")),
      });
      ...
      expect(context.readPreparedGatewayModelCatalog).toHaveBeenCalledWith({ agentId: "main" });
  ...
  test("chat.history returns the full primary response when no prepared catalog exists", async () => {
    ...
    try {
      ...
      await callDirectChat("chat.history", {
        id: "history-without-prepared-catalog",
        params: makeMainSessionParams(),
        respond: captureChatResponse(responses),
        context,
      });
      ...
      expect(responses[0]?.payload).toMatchObject({
        sessionKey: "main",
        sessionId: "sess-main",
        defaults: { modelProvider: "test-provider" },
        sessionInfo: {
          key: "agent:main:main",
          sessionId: "sess-main",
          modelProvider: "test-provider",
      ...
      expect(Array.isArray((responses[0]?.payload as { messages?: unknown })?.messages)).toBe(true);
    } finally {
      clearConfigCache();
      testState.agentConfig = undefined;
      testState.sessionStorePath = undefined;
  ...
  test("concurrent chat.history calls never fan out into provider discovery", async () => {
    openDirectChatSession();
    try {
      await writeStoredMainSession({
        modelProvider: "test-provider",
      ...
      const readPreparedGatewayModelCatalog = vi
        .fn<NonNullable<GatewayRequestContext["readPreparedGatewayModelCatalog"]>>()
        .mockResolvedValue([
          { provider: "test-provider", id: "catalog-model", name: "Catalog Model" },
        ]);
      ...
      await Promise.all(
        ["history-concurrent-1", "history-concurrent-2", "history-concurrent-3"].map((id) =>
          callDirectChat("chat.history", {
            id,
            params: makeMainSessionParams(),
      ...
      expect(readPreparedGatewayModelCatalog).toHaveBeenCalledTimes(3);
      ...
      expect(
        responses.every(
          (response) =>
            (response.payload as { sessionInfo?: { sessionId?: string } })?.sessionInfo
              ?.sessionId === "sess-main",
        ),
      ).toBe(true);
    } finally {
  ...
  test("gateway challenge and cached health stay responsive during foreground catalog reads", async () => {
    await withGatewayChatHarness(async ({ ws }) => {
      ...
      const foregroundRequests = Promise.all([
        rpcReq(ws, "chat.history", makeMainSessionParams()),
        rpcReq(ws, "models.list"),
        rpcReq(ws, "sessions.list", { limit: 100 }),
      ]);
      const healthRequest = rpcReq(ws, "health");
      const reconnect = await harness.openWs();
      try {
        const [health, challenge] = await Promise.all([
          healthRequest,
          readConnectChallengeNonce(reconnect, 2_000),
        ]);
      ...
      const foregroundResponses = await foregroundRequests;
      expect(foregroundResponses.every((response) => response.ok)).toBe(true);
    });
  });
