    ...
    await withStateDirEnv("openclaw-perf-title-batch-", async () => {
      ...
      try {
        const result = await listSessionsFromStoreAsync({
          ...
          storePath,
          store,
          opts: { includeDerivedTitles: true, includeLastMessage: true, limit: 100 },
        });

        expect(result.sessions).toHaveLength(100);
        expect(titleBatchSpy).toHaveBeenCalledOnce();
        expect(titleBatchSpy.mock.calls[0]?.[0]).toHaveLength(100);
        ...
        expect(sessionsByKey.get("agent:main:title-batch-99")).toMatchObject({
          derivedTitle: "title 99",
          lastMessagePreview: "last 99",
        });
        ...
        await listSessionsFromStoreAsync({
          ...
          storePath,
          store,
          opts: { includeDerivedTitles: false, includeLastMessage: false, limit: 100 },
        });
        expect(titleBatchSpy).toHaveBeenCalledOnce();
