...
describe("startGatewayConfigReloader", () => {
  ...
  it("reloads explicitly signaled plugin metadata when config bytes stay identical", async () => {
    ...
    const refresh = harness.reloader.notifyPluginMetadataChanged();
    await vi.runOnlyPendingTimersAsync();

    await expect(refresh).resolves.toEqual({ committed: true, generation: 1 });
  ...
  it("coalesces equivalent plugin metadata refreshes and commits the latest generation", async () => {
    const activeConfig: OpenClawConfig = {
      gateway: { reload: {} },
    };
    const readSnapshot = vi.fn(async () =>
    ...
    const harness = createReloaderHarness(readSnapshot, {
      ...
      initialInternalWriteHash: "unchanged-config",
      initialPluginInstallRecords: {},
      readPluginInstallRecords: vi.fn(async () => ({})),
    });

    const first = harness.reloader.notifyPluginMetadataChanged();
    const second = harness.reloader.notifyPluginMetadataChanged();
    await vi.runOnlyPendingTimersAsync();

    await expect(first).resolves.toEqual({ committed: true, generation: 2 });
    await expect(second).resolves.toEqual({ committed: true, generation: 2 });
    expect(readSnapshot).toHaveBeenCalledOnce();
